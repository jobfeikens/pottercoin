...
const char * const BITCOIN_CONF_FILENAME = "pottercoin.conf";
...
#else
    const char* pszModule = "pottercoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Pottercoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Pottercoin";
#else
    // Unix
    return pathRet / ".pottercoin";
#endif
#endif
}
